...
function initOrderForm() {
  ...
  form.addEventListener('submit', (event) => {
    ...
    if (isValid) {
      ...
      if (modalMessage) {
        modalMessage.textContent = 'Заявка успешно обработана и отправлена. Подробности выведены в консоль разработчика.';
      }
 ...
 * при вводе текста скрывает карточки товаров, которые не подходят под запрос.
 */
function initCatalogSearch() {
  ...
  function filterRows() {
    ...
    if (emptyMessage) emptyMessage.classList.toggle('d-none', visibleCount !== 0);
  }
